...
function atStartOfMonth(date: Date, monthOffset = 0) {
  return new Date(date.getFullYear(), date.getMonth() + monthOffset, 1);
}

function atVotingEnd(date: Date, monthOffset = 0) {
  return new Date(
    date.getFullYear(),
...
function atMonthEnd(date: Date, monthOffset = 0) {
  return new Date(
    date.getFullYear(),
...
const ideaData = [
  {
    ...
    problemStatement:
      "Code reviews are slow and inconsistent. Junior devs wait hours for feedback while senior devs are overwhelmed.",
  ...
  },
  {
    ...
    problemStatement:
      "Calendly charges $10/mo for basic features and stores user data on their servers.",
  ...
  },
  {
    ...
    problemStatement:
      "Cloud bills are opaque. Engineers don't know what they're spending until the monthly bill arrives.",
  ...
  },
  {
    ...
    problemStatement:
      "node_modules is a black hole. Devs have no idea what's actually in their dependency tree.",
  ...
  },
  {
    ...
    problemStatement:
      "Non-technical team members can't contribute to docs without learning Git.",
    targetUsers: "Open source projects, docs teams, technical writers",
    submitterId: adminUser.id,
    status: "shipped" as const,
    voteCount: 312,
    tags: ["Developer Tools", "Documentation", "CMS"],
    createdAt: ts(80),
    updatedAt: ts(20),
  },
  {
    ...
    description:
      "Point it at an OpenAPI spec and get a mock server with realistic, AI-generated responses. Hot reload on spec changes.",
  ...
  },
  {
    ...
    description:
      "Privacy-respecting habit tracker with offline sync. Data stays on your devices, syncs via CRDTs.",
    targetUsers: "Privacy-conscious users, self-improvement enthusiasts",
    submitterId: memberUser.id,
    status: "voting" as const,
    voteCount: 145,
    tags: ["Privacy", "Productivity", "Mobile"],
    createdAt: ts(10),
    updatedAt: ts(10),
  },
  {
    ...
    status: "shipped" as const,
    voteCount: 289,
    tags: ["Developer Tools", "GitHub", "Analytics"],
    createdAt: ts(70),
    updatedAt: ts(25),
  },
];
...
const projectData = [
  ...
  {
    ideaId: insertedIdeas.find((i) => i.slug === "github-insights-extension")!
      .id,
...
// --- Monthly Cycles ---
const now = new Date();
const currentMonthStatus = now.getDate() <= 7 ? "voting" : "building";

db.insert(schema.cycles)
  .values({
    startDate: atStartOfMonth(now),
    endDate: atVotingEnd(now),
    selectedIdeaId:
      currentMonthStatus === "building"
        ? insertedIdeas.find((i) => i.slug === "ai-code-review-bot")!.id
        : null,
    status: currentMonthStatus,
    createdAt: atStartOfMonth(now),
  })
  .run();

db.insert(schema.cycles)
  .values({
    startDate: atStartOfMonth(now, 1),
    endDate: atVotingEnd(now, 1),
    status: "voting",
    createdAt: atStartOfMonth(now, 1),
  })
  .run();
...
db.insert(schema.cycles)
  .values({
    startDate: atStartOfMonth(now, -1),
    endDate: atMonthEnd(now, -1),
    selectedIdeaId: insertedIdeas.find(
      (i) => i.slug === "dependency-visualizer"
    )!.id,
    status: "complete",
    createdAt: atStartOfMonth(now, -1),
  })
...
db.insert(schema.cycles)
  .values({
    startDate: atStartOfMonth(now, -2),
    endDate: atMonthEnd(now, -2),
    selectedIdeaId: insertedIdeas.find((i) => i.slug === "git-based-cms")!.id,
    status: "complete",
    createdAt: atStartOfMonth(now, -2),
  })
  .run();

console.log(
  "  Created 4 cycles (current month, next voting window, 2 complete)"
);
...
const commentData = [
  {
    ...
    body: "This would be amazing! I spend hours reviewing PRs every day.",
    createdAt: ts(40),
  },
  {
    userId: memberUser.id,
    ideaId: insertedIdeas[0].id,
    body: "Would it support custom rules per project?",
    createdAt: ts(38),
  },
  {
    userId: adminUser.id,
    ideaId: insertedIdeas[0].id,
    body: "Yes! The plan is to support .reviewrc config files.",
    createdAt: ts(37),
  },
  {
    userId: memberUser.id,
    ideaId: insertedIdeas[1].id,
    body: "I'd love a self-hosted Calendly. Take my vote!",
    createdAt: ts(28),
  },
  {
    userId: adminUser.id,
    ideaId: insertedIdeas[1].id,
    body: "The one-click deploy angle is really compelling.",
    createdAt: ts(27),
  },
  {
    userId: contributorUser.id,
    ideaId: insertedIdeas[2].id,
    body: "Multi-cloud cost tracking from the terminal? Sign me up.",
    createdAt: ts(22),
  },
  {
    userId: adminUser.id,
    ideaId: insertedIdeas[3].id,
    body: "Marp is good but this could be even better with live preview.",
    createdAt: ts(18),
  },
  {
    userId: memberUser.id,
    ideaId: insertedIdeas[4].id,
    body: "This has saved me from so many dependency issues already!",
    createdAt: ts(50),
  },
  {
    userId: contributorUser.id,
    ideaId: insertedIdeas[5].id,
    body: "We need a good open-source alternative to LaunchDarkly.",
    createdAt: ts(12),
  },
  {
    userId: adminUser.id,
    ideaId: insertedIdeas[6].id,
    body: "GitDocs is one of our best shipped projects. Great community effort!",
    createdAt: ts(30),
  },
  {
    userId: memberUser.id,
    ideaId: insertedIdeas[7].id,
    body: "AI-generated mock data is a game changer for frontend dev.",
    createdAt: ts(32),
  },
  {
    userId: contributorUser.id,
    ideaId: insertedIdeas[8].id,
    body: "CRDTs for sync is the right call. No server needed!",
    createdAt: ts(8),
  },
];
...
for (const c of commentData) {
  db.insert(schema.comments)
    .values({ ...c, updatedAt: c.createdAt })
    .run();
}
